...
if __name__ == "__main__":
    assert len(sys.argv) == 2, '[USAGE] $ python %s img_3.jpg' % (os.path.basename(__file__), INPUT)
    INPUT = sys.argv[1]
    ...
    img_1 = cv2.imread('equalized_img_3.jpg')
    img_2 = cv2.imread('grey_img_3.jpg')
    img_3 = cv2.imread('histogram.png')
    img_4 = cv2.imread('histogram_eq.png')

    cv2.imshow("equalized_img",img_1) 
    cv2.imshow("grey_img_3",img_2)
    cv2.imshow("histogram",img_3) 
    cv2.imshow("histogram_eq",img_4)
    cv2.waitKey()